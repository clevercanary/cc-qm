...
function parseConditionalQueries (filter) {
    ...
    for (var j = 0; j < filter.conditions.length; j++) {
        ...
        else if (conditionDefinition.filterType === "NOT") {

            condition = {};
            condition[conditionDefinition.fieldName] = {
                "$ne": conditionDefinition.value
            }
            conditions.push(condition);
        }
        ...
        else if (conditionDefinition.filterType === "REGEXP") {
            
            var options = filter.options;
            
            condition = {};
            condition[conditionDefinition.fieldName] = new RegExp(conditionDefinition.value, options); 
            conditions.push(condition);
        }
    }
    return conditions;
